...
fs.readdir(directoryPath, (err, dirs) => {
  ...
  const validatedValues = dirs.map((exchangeDir) => {
    const filePath = directoryPath + "/" + exchangeDir + "/info.json";
    return new Promise((resolve, _) => {
      fs.readFile(filePath, "utf8", (_, data) => {
        const parsed = JSON.parse(data);
        resolve({
          ...parsed,
          exchange_id: exchangeDir,
        });
      });
    });
  });
  Promise.all(validatedValues)
    .then(async (values) => {
      ...
      const mutation = gql`
        mutation CrateExchange($data: [ExchangeGithub!]) {
          updateOrCreateExchangeFromGithub(data: { exchanges: $data }) {
            id
          }
        }
      `;
